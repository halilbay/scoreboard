...
export class Assert {
    static fail(actual, expected, message) {
        throw new AssertionError(message || `Failed value: ${util.inspect(actual)}; ${expected !== undefined ? `Expected: ${util.inspect(expected.name || expected)}` : ''}`)
    }
    ...
    static id(value, { required = false, message = '' } = {}) {
        ...
        const isUuid = UUID_REGEXP.test(value)
        const isValidId = isPositiveInteger || isUuid
    ...
    static url(value, { required = false, message = '' } = {}) {
        Assert.string(value, { required, message })
        if (value && !URL_REGEXP(value)) Assert.fail(value, 'URL', message)
    }
}